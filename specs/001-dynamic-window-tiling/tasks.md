...
- [ ] T047 [US6] Implement Workspaces page in `workspace-tiling-window-manager/prefs.js`: on `fillPreferencesWindow` call, read `global.workspace_manager.get_n_workspaces()` (via `Gio.DBusProxy` or by importing Shell); remove stale indices (>= workspace count) from `tiling-enabled-workspaces` setting; for each workspace index (0-based) add an `Adw.SwitchRow` labelled `_('Workspace %d').format(i + 1)` with its active state bound to the corresponding index in the `tiling-enabled-workspaces` array, updating the full array on toggle
